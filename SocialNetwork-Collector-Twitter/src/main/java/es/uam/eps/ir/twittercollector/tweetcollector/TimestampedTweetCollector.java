 ...
 */
public class TimestampedTweetCollector implements TweetCollector
{
    /**
     * Initial timestamp
     */
     ...
     */
    public TimestampedTweetCollector(Date startDate, Date endDate)
    {
    ...
    public List<Status> getTimeline(long user) 
    {
        ...
        while(stop == false)
        {
            try
            {
                ...
                for(Status st : resp)
                {
                    if(st.getCreatedAt().compareTo(startDate) >= 0 && st.getCreatedAt().compareTo(endDate) <= 0)
                    {
                        status.add(st);
                    }
