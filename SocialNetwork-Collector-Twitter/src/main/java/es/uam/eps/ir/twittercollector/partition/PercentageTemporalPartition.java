 ...
 */
public class PercentageTemporalPartition extends TemporalPartition
{
    /**
     * Date of separation
     */
     ...
     */
    public PercentageTemporalPartition(String database, double percentage) throws TwitterCollectorException 
    {
        super(database);
        if(percentage < 0.0)
        {
        ...
        while(offset < numTweets)
        {
            ...
            while(tweets.isEmpty() == false)
            {
                ...
                if(orig != null) // The studied tweet is a retweet
                {
                    ...
                    {
                        if(this.train.findEdge(us.getScreenName(), cr.getScreenName()) == null)
                            this.train.addEdge(this.train.getEdgeCount()+1, us.getScreenName(), cr.getScreenName());
                    }
                    else //Add to test
                    {
                        if(this.test.findEdge(us.getScreenName(), cr.getScreenName()) == null)
